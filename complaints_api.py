...
def register_complaints_api(app):
    ...
    def get_complaints():
        ...
        try:
            ...
            if user_role == 'admin':
                ...
                if user_dept:
                    # Some datasets store department with extra context (e.g., distance).
                    # Use ilike to match both exact and suffixed department names.
    ...
    def update_complaint(complaint_id):
        """Update complaint workflow fields."""
        try:
            supabase, error_response = supabase_or_error()
            if error_response:
                return error_response
            ...
            # updated_at is optional in some schemas; avoid breaking older tables.
    ...
    def submit_citizen_feedback(complaint_id):
        """Allow the reporting citizen to confirm whether the issue was truly resolved."""
        try:
            supabase, error_response = supabase_or_error()
            if error_response:
                return error_response
    ...
    def upload_proof(complaint_id):
        """
        Upload proof of work (image + optional voice) for a complaint.
        Called by ground workers after completing a task.
        """
        try:
            supabase, error_response = supabase_or_error()
            if error_response:
    ...
    def get_user_complaints():
        """Get complaints for the logged-in citizen user."""
        try:
            supabase, error_response = supabase_or_error()
            if error_response:
                return error_response
    ...
    def assign_user_role():
        """
        Assign or update user role by email.
        Expected body: {"email": "...", "role": "officer|worker|citizen|admin", "department": "..."}
        """
        try:
            supabase, error_response = supabase_or_error()
            if error_response:
